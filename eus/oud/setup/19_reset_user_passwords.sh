...
mapfile -t COMMON_USERS < <(${OUD_INSTANCE_HOME}/OUD/bin/ldapsearch --hostname ${HOST} --port $PORT -D "${DIRMAN}" -j $PWD_FILE -b "ou=${USER_OU},${BASEDN}" "(objectClass=person)" dn|sed 's/^dn: //'|grep -i 'cn')
