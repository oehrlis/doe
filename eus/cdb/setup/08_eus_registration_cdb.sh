...
# wenn vorhanden check dn in wallet
    ...
    # check if we do have a old wallet
    WALLET_DIR="${ORACLE_BASE}/admin/${ORACLE_SID}/wallet"
    TIMESTAMP="$(date '+%y%m%d_%H%M')"
    if [ -f "${WALLET_DIR}/ewallet.p12" ]; then
        mv -v ${WALLET_DIR}/ewallet.p12 ${WALLET_DIR}/ewallet.p12_${TIMESTAMP}
    fi
    if [ -f "${WALLET_DIR}/cwallet.sso" ]; then
        mv -v ${WALLET_DIR}/cwallet.sso ${WALLET_DIR}/cwallet.sso_${TIMESTAMP}
    fi
